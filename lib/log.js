...
proto.getLogger = function (category, statType) {
    ...
    if (!this.logger[category]) {
        switch (category) {
        case '[stat]':
            // this.logger[category] = getStatLogger(this, statType);
            var that = this;
            that.logger[category] = {};
            statType.forEach(function(type){
                that.logger[category][type] = getStatLogger(that, type);
            });
            break;
        case '[email]':
...
function getStatLogger(self, type) {
    ...
    log4js.addAppender(log4js.appenderMakers.dateFile({
        filename: options.filename + '_' + options.appID + '_' + type,
